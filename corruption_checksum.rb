...
def largest_subtract_smallest(arr)
  i = 0
  largest = arr[0]
  smallest = arr[0]
...
def all_rows_largest_subtract_smallest(arr_of_arrs)
  sum = 0
  arr_of_arrs.each do |subarr|
    sum = sum + largest_subtract_smallest(subarr)
  end
  return sum
end

def all_rows_even_division_sums(arr_of_arrs)
  sum = 0
  arr_of_arrs.each do |subarr|
    sum = sum + even_division_nums(subarr)
  end
  return sum
end
...
# p even_division_nums([9,4,7,3])

p all_rows_even_division_sums(arr_of_arrs)
